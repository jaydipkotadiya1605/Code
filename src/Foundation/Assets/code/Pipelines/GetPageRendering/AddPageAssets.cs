namespace Sitecore.Foundation.Assets.Pipelines.GetPageRendering
{
    ...
    public class AddPageAssets : GetPageRenderingProcessor
    {
        ...
        protected void AddAssets(Item item)
        {
            ...
            var scriptFileHead = this.GetPageAssetValue(item, Templates.PageAssets.Fields.JavascriptFileTop);
            if (!string.IsNullOrWhiteSpace(scriptFileHead))
            {
                AssetRepository.Current.AddScriptFile(scriptFileHead, ScriptLocation.Head, true);
            }
        }
