namespace Sitecore.Foundation.Assets.Repositories
{
    ...
    public class AssetRepository
    {
        ...
        public Asset Add(Asset asset, bool preventAddToCache = false)
        {
            ...
            if (asset.AddOnceToken != null && this._items.Any(x => x.AddOnceToken != null && x.AddOnceToken == asset.AddOnceToken))
            {
                return null;
            }

            if (asset.Content != null && this._items.Any(x => x.Content != null && x.Content == asset.Content))
            {
                return null;
            }

            if (!preventAddToCache && RenderingContext.CurrentOrNull != null)
            {
                var rendering = RenderingContext.CurrentOrNull.Rendering;
                AddToCache(asset, rendering);
            }
        ...
        public Asset AddScriptFileOnTop(string file, bool preventAddToCache = false)
        {
            return this.AddScriptFile(file, ScriptLocation.Head, preventAddToCache);
        }
        ...
        private string CleanContent(string content)
        {
            ...
            if (!string.IsNullOrEmpty(content) && Configuration.Configuration.IsUseMinifyJavascript())
            {
                content = content.ToLower();
                if (!content.StartsWith("http") && !content.EndsWith(".min.js") && content.EndsWith(".js"))
                {
                    return content.Replace(".js", ".min.js");
                }
            }
            return content;
        }
